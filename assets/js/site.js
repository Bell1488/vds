(() => {
  ...
  $$('.lead-form').forEach(form=>{
    let started=false; form.addEventListener('input',()=>{if(!started){started=true;VDS.goal('lead_start',{form:form.dataset.form||'lead',lead_type:document.body.dataset.page==='payment'?'payment':'logistics'});VDS.goal('form_start',{form:form.dataset.form||'lead'})}},{once:true});
    ...
    form.addEventListener('submit',async e=>{
      ...
      VDS.goal('lead_submit',{form:form.dataset.form||'lead',lead_type:isPayment?'payment':'logistics'}); VDS.goal('form_submit',{form:form.dataset.form||'lead'}); submit.disabled=true; if(status){status.className='form-status';status.textContent='Отправляем заявку…'}
  ...
  if(calcRoot&&document.body.dataset.page==='payment'){
    ...
    const update=()=>{const amount=Math.max(0,Number(amountEl?.value||0));const estimate=rates&&amount>0?amount*Number(rates.CNY):NaN;summary.textContent=estimate?format(estimate):'—';summaryNote.textContent=estimate?'Справочно, без учёта комиссии и коммерческого курса':'Введите сумму в CNY для справочного пересчёта';sourceLabel.textContent=rates?`${rates.source||'Базовый курс'} · ${rates.updated||'дата не указана'}`:'Точный курс фиксирует менеджер перед оплатой';rateLabel.textContent=rates?`1 CNY ≈ ${Number(rates.CNY).toLocaleString('ru-RU',{maximumFractionDigits:4})} RUB · без комиссии`:'Точный курс фиксирует менеджер перед оплатой';window.VDSPaymentLead=amount>0?{confirmed:true,payment_type:service,service_name:serviceNames[service],amount_cny:amount}:null};
    amountEl?.addEventListener('input',()=>{used=true;update()}); serviceTabs.forEach(tab=>tab.addEventListener('click',()=>{serviceTabs.forEach(t=>t.classList.remove('active'));tab.classList.add('active');service=tab.dataset.payService||'supplier';used=true;update();VDS.goal('payment_service_select',{service});VDS.goal('payment_type_select',{service})}));

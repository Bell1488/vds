(() => {
  ...
  if(calcRoot&&document.body.dataset.page==='payment'){
    ...
    const ratesWidget={cny:$('#rate-cny'),usd:$('#rate-usd'),updated:$('#rates-updated span:last-child'),source:$('#rates-source')};
    const updateRatesWidget=data=>{const available=Number(data?.CNY)>0&&Number(data?.USD)>0;if(ratesWidget.cny)ratesWidget.cny.textContent=available?Number(data.CNY).toLocaleString('ru-RU',{minimumFractionDigits:2,maximumFractionDigits:4}):'—';if(ratesWidget.usd)ratesWidget.usd.textContent=available?Number(data.USD).toLocaleString('ru-RU',{minimumFractionDigits:2,maximumFractionDigits:2}):'—';if(ratesWidget.updated)ratesWidget.updated.textContent=available?`Обновлено: ${data.updated||'сегодня'}`:'Данные временно недоступны';if(ratesWidget.source&&available)ratesWidget.source.textContent=`Источник: ${data.source||'Банк России'} · справочный курс`};
    ...
    fetch('/api/rates',{headers:{Accept:'application/json'}}).then(r=>r.ok?r.json():Promise.reject()).then(d=>{updateRatesWidget(d);if(Number(d.CNY)>0)rates=d;update()}).catch(()=>{updateRatesWidget(null);rates=null;update()}); update();
